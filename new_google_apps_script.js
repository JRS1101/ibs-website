// Google Apps Script - I.B.S 문의 폼 데이터 저장용 (새 스프레드시트 버전)
// 더 예쁜 테이블 형태로 저장됩니다!

// 스프레드시트 설정
const SPREADSHEET_ID = '1Zuyq8cS2uOGL90jgjXmXNk_3wmaFGZ5fFfnM0jSmTOI'; // 새 Google Sheets ID
...
function resetPrettyTable() {
  try {
    ...
    Logger.log('❌ 테이블 생성 실패: ' + error.toString());
  }
}
...
function updateInquiryStatus(row, status) {
  try {
    ...
    Logger.log(`✅ ${row}번 문의 상태가 "${status}"로 업데이트되었습니다.`);
...
function setupSpreadsheet() {
  try {
    ...
    if (!sheet) {
      sheet = spreadsheet.insertSheet(SHEET_NAME);
      setupPrettyTable(sheet);
      Logger.log('✅ 새 스프레드시트가 설정되었습니다!');
    } else {
      Logger.log('ℹ️ 스프레드시트가 이미 설정되어 있습니다.');
    }
    
  } catch (error) {
    Logger.log('❌ 스프레드시트 설정 실패: ' + error.toString());
  }
} 