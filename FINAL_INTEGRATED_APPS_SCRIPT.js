// 🎯 IBS 통합 견적 관리 시스템 - Google Apps Script 최종 배포용
...
function doPost(e) {
  try {
    ...
    if (!e.postData || !e.postData.contents) {
      return createResponse({ error: 'POST 데이터가 없습니다.' });
    }
...
function getQuotationList() {
  try {
    ...
    if (!sheet) {
      console.log('⚠️ 견적서 시트를 찾을 수 없습니다.');
      return createResponse({ quotations: [] });
    }
    
    const data = sheet.getDataRange().getValues();
    if (data.length <= 1) {
      console.log('⚠️ 견적서 데이터가 없습니다.');
      return createResponse({ quotations: [] });
    }
    ...
    const quotations = rows
      ...
      .map(row => ({
        quoteNumber: row[1],        // B열: 견적번호
        quoteDate: row[2],          // C열: 견적일자
        projectName: row[4],        // E열: 프로젝트명
        clientCompany: row[5],      // F열: 고객업체
        clientContact: row[6],      // G열: 담당자
        totalAmount: row[37] || 0,  // AL열: 총계약금액
        status: row[38] || '진행중', // AM열: 상태
        items: [
          {
    ...
    console.log(`✅ 견적서 목록 조회 성공: ${quotations.length}건`);
    return createResponse({ quotations });
...
function getQuotationData(quoteNumber) {
  try {
    if (!quoteNumber) {
      return createResponse({ error: '견적번호가 필요합니다.' });
    }
    ...
    if (!sheet) {
      return createResponse({ error: '견적서 시트를 찾을 수 없습니다.' });
    }
    ...
    if (!quotationRow) {
      return createResponse({ error: `견적서 ${quoteNumber}를 찾을 수 없습니다.` });
    }
    ...
    console.log(`✅ 견적서 데이터 조회 성공: ${quoteNumber}`);
...
function getQuotationStats(spreadsheet, today, thisMonth) {
  ...
  const monthlyQuotes = rows
    .filter(row => {
      const quoteDate = new Date(row[2]);
      return Utilities.formatDate(quoteDate, 'Asia/Seoul', 'yyyy-MM') === thisMonth;
    })
  ...
  return {
    ...
    notifications: [
      ...
      { type: 'success', message: `이번 달 견적액 ${monthlyQuotes.toLocaleString()}원` }
    ]
  };
}
...
function getTransactionStats(spreadsheet, today, thisMonth) {
  ...
  return {
    ...
    notifications: [
      ...
      { type: 'success', message: `배송 완료율 ${Math.round(deliveryRate)}%` }
    ]
  };
}
...
function getInvoiceStats(spreadsheet, today, thisMonth) {
  ...
  return {
    ...
    notifications: [
      ...
      { type: 'success', message: `결제 완료율 ${Math.round(paymentRate)}%` }
    ]
  };
}
...
// 🔧 테스트 함수들
function testGetQuotationList() {
  const result = getQuotationList();
  console.log('견적서 목록 테스트:', result.getContent());
}

function testGetQuotationData() {
  const result = getQuotationData('IBS-2025-001');
  console.log('견적서 데이터 테스트:', result.getContent());
}

function testGetStats() {
  const result = getAnalyticsStats('quotation');
  console.log('통계 데이터 테스트:', result.getContent());
} 